export default function progress(){
    document.querySelector('.disa').addEventListener('click', function(){ 
        document.querySelectorAll('.ExpensesActive').forEach((elem)=>{
            // document.querySelector('.progress').insertAdjacentHTML('afterbegin', `<div class="${elem.id} Progress" role="progressbar" style="width: ${document.querySelector('.spentMoney').textContent == String(0) ? '100%' : Math.round((+elem.querySelector('.rasxoMoney').textContent.replace(/[a-zа-яё]/gi, '') / +document.querySelector('.spentMoney').textContent) * 100)+'%'}; background-color: ${}; color: black" aria-valuenow="30" aria-valuemin="0" aria-valuemax="100">${el.id}</div>`)
        })
